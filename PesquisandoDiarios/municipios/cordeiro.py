...
class Cordeiro:
    def __init__(self, pesquisa, data_inicial, data_final):
        ...
        self.url = 'https://transparencia.cordeiro.rj.gov.br/jornal.php?Num=&Sec=&dtini=' + data_inicial + \
    ...
    def retornaDiarios(self):
        ...
        while True:
            ...
            if len(lista_a) == 0:
                break
            for a in lista_a:
                url = 'https://transparencia.cordeiro.rj.gov.br/' + a['href']
                site = requests.get(url)
                site = BeautifulSoup(site.text, "html.parser")

                codigo = site.find('a', class_='btn btn-primary btn-sm pull-right')['href']
                link = 'https://transparencia.cordeiro.rj.gov.br/' + codigo.replace(' ', '%20')
